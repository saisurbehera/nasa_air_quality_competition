...
def preprocess(url, gridCellDF, datasets, grid_type=None, total_bounds=None):
    fpth = str(RAW / '/'.join(url.split('/')[-4:]))
    hdf = SD(fpth, SDC.READ)
    ...
    elif grid_type == 'grid2':
        td = hdf.select('cosSZA')
    else:
        raise Exception('Incorrect grid type at preprocess')
    ...
    for dataset_name in datasets:
        ...
        data_shape = hdf.select(dataset_name).info()[2]
        calibration_dict = data.attributes()
        dataDict[dataset_name] = calibrate(dataset=data, shape=data_shape, calibration_dict=calibration_dict)
...
def parallel_process(granulePaths, gridCellDF, datasets, grid_type=None, n_jobs=n_cpus//2):
    args = ((gp, gridCellDF, datasets, grid_type) for gp in granulePaths)
...
def calculate_features(feature_df, labels_df, datasets, type_=None):
    ...
    how = "left" if type_ == "train" else "left"
